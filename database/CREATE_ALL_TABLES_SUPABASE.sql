-- ============================================================================
-- SCRIPT COMPLET : Création de toutes les tables pour Supabase
-- ============================================================================
-- Ce script crée toutes les tables dans le bon ordre (respect des dépendances)
-- À exécuter dans Supabase Dashboard → SQL Editor
-- ============================================================================
-- Date : 2025
-- Base de données : Blossom Cafe
-- ============================================================================
...
ALTER TABLE "app_settings" ADD CONSTRAINT "app_settings_setting_type_check" 
  CHECK ("setting_type" IN ('boolean', 'string', 'number', 'json'));
...
ALTER TABLE "news" ADD CONSTRAINT "news_size_check" 
  CHECK ("size" IN ('normal', 'large', 'extra-large'));
...
ALTER TABLE "users" ADD CONSTRAINT "users_role_check" 
  CHECK ("role" IN ('client', 'manager', 'admin', 'kiosk'));

-- Mettre à jour settings pour ajouter la clé étrangère vers users
ALTER TABLE "settings" ADD CONSTRAINT "settings_ibfk_1" 
...
ALTER TABLE "shopping_list" ADD CONSTRAINT "shopping_list_status_check" 
  CHECK ("status" IN ('pending', 'ordered', 'received'));
ALTER TABLE "shopping_list" ADD CONSTRAINT "shopping_list_priority_check" 
...
ALTER TABLE "loyalty_rewards" ADD CONSTRAINT "loyalty_rewards_reward_type_check" 
  CHECK ("reward_type" IN ('percentage', 'product', 'fixed'));
...
ALTER TABLE "inventory_logs" ADD CONSTRAINT "inventory_logs_change_type_check" 
  CHECK ("change_type" IN ('purchase', 'sale', 'adjustment', 'return', 'waste'));
...
ALTER TABLE "orders" ADD CONSTRAINT "orders_payment_method_check" 
  CHECK ("payment_method" IN ('cash', 'card', 'stripe', 'paypal', 'mixed'));
...
ALTER TABLE "order_payments" ADD CONSTRAINT "order_payments_method_check" 
  CHECK ("method" IN ('cash', 'card', 'stripe', 'paypal', 'mixed', 'voucher', 'other', 'check', 'transfer'));
...
ALTER TABLE "loyalty_transactions" ADD CONSTRAINT "loyalty_transactions_transaction_type_check" 
  CHECK ("transaction_type" IN ('earned', 'redeemed', 'bonus', 'expired', 'adjustment'));
...
-- FIN DU SCRIPT
-- ============================================================================
-- Toutes les tables ont été créées dans le bon ordre
