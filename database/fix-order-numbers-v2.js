/**
 * Script pour corriger les numéros de commande
 * Supprime temporairement l'index unique, met à jour, puis le recrée
 * Format: CMD-XXXX (4 chiffres, réinitialisé chaque jour)
 */
...
async function fixOrderNumbers() {
  ...
  try {
    ...
    console.log('🔄 Correction des numéros de commande...\n');

    // Récupérer les informations sur l'index unique
    ...
    // Supprimer temporairement l'index unique s'il existe
    if (indexName) {
    ...
    // Étape 1: Mettre TOUTES les commandes en temporaire
    ...
    for (const dateStr of sortedDates) {
      ...
      for (let i = 0; i < orders.length; i++) {
        ...
        if (i < 3 || i === orders.length - 1) {
          console.log(`  ✅ ${oldNumber} → ${newOrderNumber}`);
        }
      }
      console.log('');
    }
    ...
    sample.forEach(order => {
      console.log(`  ${order.order_number} - ${order.date_commande}`);
    });
