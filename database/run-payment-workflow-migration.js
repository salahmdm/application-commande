...
const logger = require('./utils/logger');
    ...
    logger.log('🚀 Migration payment_workflow : démarrage');

    if (!(await columnExists(connection, 'orders', 'amount_paid'))) {
      logger.log('➕ Ajout colonne orders.amount_paid');
    ...
    if (!(await columnExists(connection, 'orders', 'change_amount'))) {
      logger.log('➕ Ajout colonne orders.change_amount');
      await connection.query(
    ...
    logger.log('🧮 Mise à jour des montants existants');
    await connection.query(`
      UPDATE orders
